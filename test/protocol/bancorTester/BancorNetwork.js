/**
 * Original test code:
 * https://github.com/bancorprotocol/contracts/blob/v0.4.4/solidity/test/BancorNetwork.js
...
const bancorNetworkTestCase = (artifact) => {
  const Whitelist = artifacts.require('Whitelist.sol')
  // const BancorNetwork = artifacts.require('BancorNetwork.sol')
  const BancorNetwork = artifact
  ...
  */
  ...
  contract(`${artifact.contractName} is BancorNetwork`, accounts => {
    ...
    it('verifies that getReturnByPath returns the correct amount for cross connector conversion', async () => {
      await converter2.quickConvert([etherToken.address, smartToken1.address, smartToken1.address], 1000, 1, {
        from: accounts[1],
        value: 1000
      })
    ...
    it('verifies that getReturnByPath returns the correct amount for selling the smart token with a long conversion path', async () => {
      await converter4.quickConvert([etherToken.address, smartToken1.address, smartToken1.address, smartToken2.address, smartToken3.address], 1000, 1, {
        from: accounts[1],
        value: 1000
      })
    ...
    it('verifies multiple convertFor with multiple paths', async () => {
      ...
      await bancorNetwork.convertForMultiple(smartTokenBuyPathMultiple, [0, 5, 8], [10000, 10000, 10000], [1, 1, 1], accounts[0], {
        from: accounts[0],
        value: 10000
      })
    ...
    it('verifies multiple convertFor with multiple paths that each of them starts with ether', async () => {
      ...
      await bancorNetwork.convertForMultiple(smartTokenBuyPathMultiple, [0, 5], [10000, 10000], [1, 1], accounts[1], {
        from: accounts[1],
        value: 20000
      })
    ...
    it('verifies prioritized quick buy with trusted signature', async () => {
      ...
      let soliditySha3 = web3Utils.soliditySha3(maximumBlock, gasPrice, accounts[1], converter1.address, 100, {
        'type': 'address',
        'value': smartToken1BuyPath
      })
      let result = sign(soliditySha3, trustedAddress)

      await converter1.quickConvertPrioritized(smartToken1BuyPath, 100, 1, maximumBlock, result.v, result.r, result.s, {
        from: accounts[1],
        value: 100
      })
      let newBalance = await smartToken1.balanceOf.call(accounts[1])
    ...
    it('should throw when attempts to call quick convert prioritized with wrong amount', async () => {
      try {
        ...
        await converter1.quickConvertPrioritized(smartToken1BuyPath, 200, 1, maximumBlock, result.v, result.r, result.s, {
          from: accounts[1],
          value: 100
        })
        assert(false, 'didn\'t throw')
    ...
    it('should throw when attempts to call quick convert prioritized with higher block number than what appears in the signing data', async () => {
      try {
        ...
        await converter1.quickConvertPrioritized(smartToken1BuyPath, 100, 1, wrongBlockNumber, result.v, result.r, result.s, {
          from: accounts[1],
          value: 100
        })
        assert(false, 'didn\'t throw')
